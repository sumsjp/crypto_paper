...
本研究主要從 **市場微觀結構（Market Microstructure）** 和 **價格發現（Price Discovery）** 的角度，探討 **美元/比特幣（USD/BTC）交易市場的資訊傳遞機制**。具體而言，本研究的核心關注點包括：

### **1. 市場微觀結構（Market Microstructure）**
- **市場內部資訊流動（Information Flow within the Market）**
  - 透過 **Geweke 反饋測度（Geweke feedback measures）**，分析交易所之間的資訊傳遞模式，了解不同交易所的資訊主導地位與市場影響力。
  - 研究不同交易所的交易量、價格變動與資訊回饋關係，揭示市場內部資訊不對稱的情況。

- **市場碎片化（Market Fragmentation）**
  - 透過分析不同規模的交易所，研究市場的資訊整合程度（即 **市場是否高度集中於少數幾個交易所，或資訊分散在多個市場中**）。
  - 分析「**主導交易所（Dominant Exchange）**」與「**衛星交易所（Satellite Exchange）**」的互動模式，判斷交易所之間是否存在資訊領導者。

### **2. 價格發現（Price Discovery）**
- **主要交易所的資訊領導地位（Information Leadership）**
  - 研究 **哪些交易所是價格發現的主要驅動者**，例如本研究發現 **Bitfinex 是資訊傳遞的核心**。
  - 檢驗價格變動是否主要來自交易量最大的交易所，或是否受到市場整體變動的影響。

- **交易量與價格資訊的關聯（Trading Volume and Price Information）**
  - 探討交易量在市場資訊傳遞中的角色，即 **是否「交易量越大，價格發現效率越高」**。
  - 研究主要與次要交易所之間的資訊滯後關係，檢視市場的資訊效率（Information Efficiency）。

- **波動性與市場資訊整合（Volatility and Market Integration）**
  - 檢驗價格波動性（Volatility）如何影響市場資訊傳遞，是否 **主要交易所的波動性會提升市場整合度**，或 **較小交易所的波動性會引入額外市場雜訊**。

### **3. 事件衝擊與市場反應（Event Impact and Market Reaction）**
- 研究涵蓋 **Mt.Gox 破產（2014）** 及 **Bitfinex 遭駭（2016）** 這兩大市場動盪事件：
  - **這些事件如何影響比特幣市場的價格發現機制？**
  - **市場資訊主導地位是否因事件發生而改變？**
  - **市場在事件發生後的資訊整合度是否下降或提升？**

### **4. 交易所間的競爭與市場結構（Exchange Competition and Market Structure）**
- 研究市場競爭如何影響價格發現，例如：
  - **是否所有交易所都能有效反映市場資訊？**
  - **市場是否逐漸趨向「資訊集中於少數幾
