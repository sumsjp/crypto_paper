...
研究結果顯示，在加密貨幣市場中，時間序列動能（Time-Series Momentum）與橫截面動能（Cross-Sectional Momentum）展現出不同的特徵與可行性：

**時間序列動能**
1. **強烈的時間序列動能**：當市場處於上升趨勢時，時間序列動能策略能夠獲得顯著的回報。特別是當回溯期（look-back period）為 28 天，持有期（holding period）為 5 天時，策略表現最佳，其夏普比率（Sharpe ratio）高達 **1.51**，而市場投資組合的夏普比率僅為 **0.85**。
2. **市場依賴性**：時間序列動能主要發生在市場上漲時，市場下跌時並未觀察到顯著的動能效果。這與股市中的時間序列動能不同，顯示加密貨幣市場具有不同的行為模式。
3. **過度反應機制（Overreaction Mechanism）**：回歸分析顯示，時間序列動能與**持續過度反應因素（TS-CO factor）**具有高度相關性（t-值達 **6.52**）。這表明市場參與者可能因為情緒波動過大，導致價格短期內持續上漲或下跌，形成動能。
4. **不同資產類型的一致性**：時間序列動能策略在不同市值、交易量和投資者關注度的資產群組中均表現一致，顯示該現象普遍存在於整個加密貨幣市場。
5. **交易成本影響較小**：由於該策略並非每天交易，因此即使考慮 **0.15%** 的交易成本，其回報仍然優於市場投資組合。

**橫截面動能**
1. **橫截面動能證據較弱**：當所有加密貨幣納入考慮時，回歸分析顯示多數情況下出現**反轉效應（Reversal Effect）**而非動能效應。
2. **大型加密貨幣展現動能效應**：針對市值排名前 5% 的加密貨幣進行回歸分析後，觀察到較強的橫截面動能，特別是在短期回溯（小於 7 天）與短期持有（小於 14 天）期間動能效果較明顯。
3. **多數動能投資組合最終被清算**：選取 21 組最有可能產生動能的回溯期與持有期進行投資組合回測，發現 **5 組** 於樣本期間內清算（損失超過 100%），顯示市場極端波動對動能策略影響極大。
4. **長倉（Long-Only）表現優於長短倉（Long-Short）**：動能策略的主要利潤來自於**做多強勢資產**，而**做空弱勢資產則經常產生虧損**，這與股票市場的動能效應相反。
5. **最佳橫截面動能策略**：
   - 最佳的 **長短倉（Long-Short）策略** 為 **(1,7)**，其夏普比率為 **1.31**，而市場為 **0.85**。
   - **(14,7) 策略累積回報率最高**，達到 **101,218%**，但期間內有三個子投資組合被清算。

**整體結論**
- **時間序列動能明顯且可行**，但主要發生於市場上升趨勢，且其盈利機制與市場過度反應有關。
- **橫截面動能證據較弱**，僅對大型加密貨幣有效，而大多數加密貨幣顯示價格反轉效應。
- **做空策略風險極高**，即使回歸分析顯示存在動能效應，實際投資結果仍然常遭遇清算，顯示短倉在加密貨幣市場中的可行性較低。
- **極端波動是主要風險**，過去研究未充分考慮清算風險，本研究透過每日市值調整與考慮交易成本，更貼近真實交易情境，顯示動能策略仍需謹慎操作。

這項研究對於加密貨幣市場的動能現象提供了新的見解，揭示了過去研究可能忽略的關鍵風險因素，同時也顯示在市場趨勢較為明確時，動能策略仍然有潛在的獲利機會。
